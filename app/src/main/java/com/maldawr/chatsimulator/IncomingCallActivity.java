...
public class IncomingCallActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b);setShowWhenLocked(true);setTurnScreenOn(true);getWindow().setStatusBarColor(0xFF0B141A);getWindow().setNavigationBarColor(0xFF0B141A);Store.ensureSeeded(this);bot=Store.getBot(this,getIntent().getLongExtra("bot_id",-1));if(bot==null){finish();return;}buildUi();startRinging();}
    private void buildUi(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER_HORIZONTAL);root.setPadding(Ui.dp(this,22),Ui.dp(this,40),Ui.dp(this,22),Ui.dp(this,34));root.setBackgroundColor(0xFF0B141A);TextView sim=Ui.label(this,"SIMULATED CALL",11,true);sim.setTextColor(0xFF25D366);sim.setGravity(Gravity.CENTER);root.addView(sim,new LinearLayout.LayoutParams(-1,Ui.dp(this,30)));TextView type=Ui.label(this,"Incoming simulated call",15,false);type.setTextColor(0xFF8696A0);type.setGravity(Gravity.CENTER);root.addView(type,new LinearLayout.LayoutParams(-1,Ui.dp(this,42)));LinearLayout.LayoutParams avp=new LinearLayout.LayoutParams(Ui.dp(this,132),Ui.dp(this,132));avp.setMargins(0,Ui.dp(this,20),0,Ui.dp(this,18));root.addView(Ui.avatar(this,bot,132),avp);TextView name=Ui.label(this,bot.name,29,true);name.setTextColor(Color.WHITE);name.setGravity(Gravity.CENTER);root.addView(name);TextView sub=Ui.label(this,bot.groupChat?bot.groupSubtitle:bot.phone,15,false);sub.setTextColor(0xFF8696A0);sub.setGravity(Gravity.CENTER);root.addView(sub);state=Ui.label(this,"Ringing…",14,false);state.setTextColor(0xFF8696A0);state.setGravity(Gravity.CENTER);LinearLayout.LayoutParams sp=new LinearLayout.LayoutParams(-1,Ui.dp(this,70));root.addView(state,sp);View spacer=new View(this);root.addView(spacer,new LinearLayout.LayoutParams(1,0,1));LinearLayout actions=new LinearLayout(this);actions.setGravity(Gravity.CENTER);FrameLayout decline=roundAction(0xFFE53935,IconView.PHONE);decline.setRotation(135);decline.setOnClickListener(v->endCall());answer=roundAction(0xFF25D366,IconView.PHONE);answer.setOnClickListener(v->{if(answered)endCall();else answerCall();});LinearLayout.LayoutParams dp=new LinearLayout.LayoutParams(Ui.dp(this,74),Ui.dp(this,74));dp.setMargins(0,0,Ui.dp(this,78),0);actions.addView(decline,dp);actions.addView(answer,new LinearLayout.LayoutParams(Ui.dp(this,74),Ui.dp(this,74)));root.addView(actions);LinearLayout labels=new LinearLayout(this);labels.setGravity(Gravity.CENTER);TextView d=Ui.label(this,"Decline",13,false);d.setTextColor(Color.WHITE);d.setGravity(Gravity.CENTER);TextView a=Ui.label(this,"Answer",13,false);a.setTextColor(Color.WHITE);a.setGravity(Gravity.CENTER);LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(Ui.dp(this,100),Ui.dp(this,38));lp.setMargins(0,0,Ui.dp(this,52),0);labels.addView(d,lp);labels.addView(a,new LinearLayout.LayoutParams(Ui.dp(this,100),Ui.dp(this,38)));root.addView(labels);setContentView(root);}
    ...
    private void startRinging(){try{String s=Store.getCallSound(this);Uri u=s.isEmpty()?RingtoneManager.getDefaultUri(RingtoneManager.TYPE_RINGTONE):Uri.parse(s);ringtone=RingtoneManager.getRingtone(this,u);if(ringtone!=null)ringtone.play();}catch(Exception ignored){}}
    ...
    private void endCall(){stopRinging();NotificationHelper.cancelCall(this,bot);long now=System.currentTimeMillis();if(answered)Store.addCall(this,new Store.CallItem(now,bot.id,"incoming",now,(int)Math.max(1,(now-answeredAt)/1000)));else Store.addCall(this,new Store.CallItem(now,bot.id,"missed",now,0));finish();}
